package com.sathwikhbhat.reponyx.service.indexing;

import com.sathwikhbhat.reponyx.entity.IndexStatus;
import com.sathwikhbhat.reponyx.entity.Repository;
import com.sathwikhbhat.reponyx.exceptions.BadRequestException;
import com.sathwikhbhat.reponyx.exceptions.NotFoundException;
import com.sathwikhbhat.reponyx.repository.RepositoryRepository;
import com.sathwikhbhat.reponyx.service.CryptoService;
import com.sathwikhbhat.reponyx.service.UserService;
import com.sathwikhbhat.reponyx.service.ai.RagSettings;
import com.sathwikhbhat.reponyx.service.github.GithubApiClient;
import com.sathwikhbhat.reponyx.service.github.GithubRateLimiter;
...
public class IndexingService {
    ...
    private void doIndex(UUID repoId, UUID userId) {
        Repository repo = repositoryRepository.findById(repoId)
                .orElseThrow(() -> new NotFoundException("Repository not found"));
        String token = cryptoService.decrypt(userService.findById(userId).getAccessToken());
        ...
        updateProgress(repoId, filePaths.size(), 0, 0, null);
        ...
        for (String path : filePaths) {
            ...
            if (processed % PROGRESS_EVERY_N_FILES == 0 || processed == filePaths.size()) {
                updateProgress(repoId, filePaths.size(), processed, totalChunks, null);
            }
            rateLimiter.pause();
        }
    ...
    private void deleteExistingVectors(String repoId) {
        try {
            Filter.Expression filter = new FilterExpressionBuilder().eq(RagSettings.METADATA_REPO_ID, repoId).build();
    ...
    @Transactional
    protected void updateProgress(UUID repoId, int total, int processed, int chunks, String error) {
